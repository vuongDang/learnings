...
- Distributional Equivalence
  - $$ P\_{M_1}(y | x) = P{M_2}(y | x)$$ for $x \in \mathcal{X}$ and $y \in \mathcal{Y}$
  - edge cases can mismatch
  - it should be correct on input that match training data
  - this is impossible to prove in practice and what is used is **epsilon-bounded distributional equivalence**
    - $$ D(P\_{M_1}( . | x) - P{M_2}( . | x)) <= \epsilon $$
...
**Why it matters?** the precision loss may be exploited as this paper showed
...
- we have to find how to compute that since $y$ is vector of floats and $y'$ a vector of integers
  - would a simple integer to floats conversion work?
    - maybe depending on the quantization technique **TODO**
